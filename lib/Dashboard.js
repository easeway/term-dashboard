...
var RowRenderer = Class({
    ...
    render: function (data, term) {
        ...
        var x = 0;
        term.horizontalAbsolute().eraseLine();
...
var Dashboard = Class({
    ...
    constructor: function (opts) {
        ...
        this._stream.on('resize', this._redraw.bind(this));
    },
    
    /** @property
     * @description Get terminal object
     */
    get term () {
        return this._term;
    },
     ...
     */
    update: function (slot, index, viewName, data) {
        slot = this._slots[slot];
        var view = this._views[viewName];
        if (slot && (view || !viewName) && index >= 0 && index < slot.rows) {
            var saved = (this._data[slot.name] || (this._data[slot.name] = []));
    ...
    refresh: function (layout) {
        layout ? this._setupLayout(layout) : this._redraw();
        return this;
    },
